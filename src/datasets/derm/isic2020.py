...
class ISIC2020(Dataset):
    ...
    def find_data(self):
        ...
        folder = os.path.join(self.root, 'train')
    ...
    def build_index(self):
        ...
        self.labels = (df['benign_malignant'] == 'malignant').to_numpy().astype(int)
        print('Done')
    ...
    def to_qa(self):
        ...
        for i in range(len(self)):
            label = self.labels[i]
            file_name = self.fnames[i].replace('ISIC_2020_Training', 'train')
            relative_path = os.path.relpath(file_name, self.root)
            assert os.path.exists(file_name)

            # Create a question for each label
            diagnosis_question = 'Is the skin lesion benign or malignant? Answer with one word from the following: Benign\nMalignant\n'
            diagnosis_answer = 'Malignant' if label == 1 else 'Benign'
    ...
    def __getitem__(self, index: int) -> Any:
        fname = self.fnames[index].replace('ISIC_2020_Training', 'train')
